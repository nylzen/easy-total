...
export const Form = () => {
  ...
  const handleOpenCategoryModal = () => {
    ...
    if (!dailyStatus.canUse) {
      toast.error(`Límite diario alcanzado (15 categorizaciones). Intenta mañana.`);
      return;
    }
  ...
  const handleConfirmWithAI = async () => {
    ...
    try {
      ...
      await navigator.clipboard.writeText(categorizedMessage);

      // Luego compartir a WhatsApp
      ...
      toast.success("Lista categorizada, copiada al portapapeles y compartida a WhatsApp 🎉");
    } catch (error) {
      ...
      try {
        // Copiar al portapapeles el mensaje simple
        ...
        window.open(whatsappUrl, "_blank");
        setIsCategoryModalOpen(false);
        toast.success("Lista copiada al portapapeles y compartida a WhatsApp (sin categorizar)");
      } catch (shareError) {
  ...
  const handleConfirmWithoutAI = async () => {
    try {
      ...
      setIsCategoryModalOpen(false);
      toast.success("Lista copiada al portapapeles y compartida a WhatsApp 📱");
    } catch (error) {
      toast.error("Error al compartir el mensaje");
            ...
            {products.length > 0 && (
              ...
              {isCategorizingProducts ? (
                ...
                <>
                  ...
                  <span className="hidden xs:inline">Compartir a WhatsApp</span>
